            ...
            if (httpRequest.status == '200') {
                ...
                if (data.AuthenticateUserRes.Status.Code == '200') {
                    ...
                    if (check_network() == 'No network connection' || check_network() == 'Unknown connection') {
					    ...
					    devicetokenregistry(localStorage.getItem('sessionToken'),url);
 ...
 */
function devicetokenregistry(sessionTokenID,url) {
    
    
    var httpRequest = new XMLHttpRequest();
	
		window.regID.setRegValueIfPresent () ;
		var regIDfromJava = window.regID.getRegValue();
			
			if ((regIDfromJava) && (regIDfromJava != ""))
			{
						
    var _object = {
        "Tid": Math.floor(Math.random() * 9000) + 1000,
        "SessionTokenId": sessionTokenID,
        "RegistrationId": regIDfromJava
    };
    var object = {
        ...
        if (httpRequest.readyState == '4') {
            if (httpRequest.status == '200') {
                ...
                if (data.RegisterAppRes.Status.Code === '200') {
                  window.regID.set3rdPartyRegsiter();	
