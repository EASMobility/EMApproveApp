...
- (BOOL)application:(UIApplication*)application didFinishLaunchingWithOptions:(NSDictionary*)launchOptions
{
    ...
    if (url && [url isKindOfClass:[NSURL class]]) {
        ...
        NSLog(@"emApprove launchOptions = %@", url);
    }
...
// only valid if emApprove-Info.plist specifies a protocol to handle
...
// this delegate method gets called if registraiton gets successfull with APNS.
...
- (void)application:(UIApplication*)application didRegisterForRemoteNotificationsWithDeviceToken:(NSData*)deviceToken
{
    ...
    NSLog(@"APN device token: %@", self.token);
    
}

// this delegate method gets called if registraiton gets failed with APNS.
...
// This delegate method gets called when app receive any notification from client server.
- (void)application:(UIApplication *)application didReceiveRemoteNotification:(NSDictionary *)userInfo {
    
    
    [self performSelector:@selector(didReceiveNotification) withObject:nil afterDelay:2.0];
    
}

-(void)didReceiveNotification {
    
    UIWebView *deviceWebView = [(MainViewController*)(self.viewController) webView];
    NSString *notificationFlag = @"True";
    NSString *function = [[NSString alloc] initWithFormat: @"NotificationCalled(\"%@\")", notificationFlag];
    
    [deviceWebView stringByEvaluatingJavaScriptFromString:function];
}
