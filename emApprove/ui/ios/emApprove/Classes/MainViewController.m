...
-(void)viewDidAppear: (BOOL)animated {
    ...
    [self performSelector: @selector(showDeviceToken) withObject:nil afterDelay:5.0];
}

-(void)showDeviceToken {
    
    NSString *deviceToken_ = [(AppDelegate*)[[UIApplication sharedApplication] delegate] token];
    NSLog(@"SendDeviceToken %@",deviceToken_);
    
    if (deviceToken_) {
        NSString *function = [[NSString alloc] initWithFormat: @"IOSDeviceToken(\"%@\")", deviceToken_];
